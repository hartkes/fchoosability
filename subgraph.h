...
int ConnectedSubgraph::next(const bitarray additional_constraints)
    ...
    // It will be &ed with the eligible_vertices.
    ...
    while (true)
    {
        ...
        for (int i=n-1; i>=0; i--)
        {
            ...
            layer[cur_layer+1].layer_universe |= 
                ( (((layer[cur_layer].layer_subset&mask)==0)-1)  // i is set in layer[cur_layer].layer_subset
                 &(neighbors[i] & (~layer[cur_layer].union_universe)) );
            mask>>=1;
        }
